...
where $P(A_i=a|Z_i=z) > 0$ and $P(m=M_i^{A_i = a}|A_i=a,Z_i=z) > 0$ for $a = 0, 1$, and, all $z \in \mathcal{Z}$ and $m \in \mathcal{M}$.
...
3. For each chain of the simulation: create a new dataset, of size $sim$, by taking a random sample, of the rows, with replacement,from the original data. The probability of selection for each row is given by $\omega_i$; $\omega = (\omega_1, \omega_2,...\omega_n) \sim Dir(1,1,...,1)$.
...
5. Using the duplicated datasets created in step 4, simulate new values of the mediator for each subject. Simulate these new mediator values using the estimated parameters from step 2. This step will result in two variables, the mediator under treatment $a$, ($M^a$), and the mediator under treatment $a'$, ($M^{a'}$). 
...
<br />
```{r visualize_posterior_graphs1, include = TRUE, echo = FALSE}
## results
#print(med_hist)
...
<br />
```{r visualize_posterior_graphs2, include = TRUE, echo = FALSE}
#print(out_hist)
knitr::include_graphics('README_files/figure-gfm/visualize_posterior_graphs2-1.png')
```

<br />
```{r visualize_posterior_graphs3, include = TRUE, echo = FALSE}
#print(dens_plot)
